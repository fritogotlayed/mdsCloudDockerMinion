...
const findEntrypoint = (dir) => new Promise((resolve, reject) => {
...
const prepSourceForContainerBuild = async (localPath, entryPoint) => new Promise((res, rej) => {
  const logger = globals.getLogger();
  const fdkInstallCmd = 'touch requirements.txt && echo "\nfdk==0.1.21" >> requirements.txt';
  const entryPointFileName = 'mdsEntry.py';

  // Install provider entry point kit
  shelljs.exec(fdkInstallCmd, { cwd: localPath, silent: true }, (retCode, sdtOut, stdErr) => {
    if (retCode === 0) {
      ...
      fs.writeFile(entryFilePath, renderedTemplate, (err) => {
        if (err) {
          ...
          const templateBody = dockerfileTemplate.generateTemplate(entryPointFileName);
