...
const self = {
  loadDefinition: () =>
    protoLoader
      ...
      .then(
        (definition) =>
          grpc.loadPackageDefinition(definition).mdsCloud.dockerMinion,
      ),
   ...
   */
  createClientFromGrpcDefinition: (definition, hostIp) =>
    new definition.Interchange(
      `${hostIp}:50051`,
   ...
   */
  invoke: async ({ definition, hostIp, payload, tries, userId, userToken }) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      client.process(
        ...
        (err, resp) => {
          ...
          const mappedResponse = self.convertUserResponseToObject(
            resp.userResponse,
          );
